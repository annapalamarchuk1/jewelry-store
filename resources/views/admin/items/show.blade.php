    ...
    <div style="flex: 1;"> 
        ...
        <p><strong>ID:</strong> {{ $item->id }}</p> 
            ...
            <div style="flex: 0 0 400px;"> 
                @if($item->image) <img src="{{ asset($item->image) }}"    alt="{{ $item->title }}" width="400" 
                style="border: 1px solid #ccc; border-radius: 5px; box-shadow: 2px 2px 8px rgba(0,0,0,0.2);"> 
                @endif 
            </div> </div> <div style="text-align: center; margin-top: 20px;"> <a href="{{ route('admin.items.index') }}" class="btn btn-secondary">Назад</a> 
        </div> 
        @endsection